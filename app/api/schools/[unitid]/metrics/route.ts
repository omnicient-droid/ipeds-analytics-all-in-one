...
export async function GET(req: NextRequest, { params }: { params: Promise<{ unitid: string }> }) {
  ...
  const unitid = Number(resolvedParams.unitid ?? req.nextUrl.searchParams.get('unitid'))
  const metric = String(req.nextUrl.searchParams.get('metric') ?? '')
  if (!unitid || !metric)
    return NextResponse.json({ error: 'unitid and metric are required' }, { status: 400 })

  const u = await prisma.university.findFirst({ where: { unitid }, select: { id: true } })
  const m = await prisma.metric.findUnique({
    where: { code: metric },
