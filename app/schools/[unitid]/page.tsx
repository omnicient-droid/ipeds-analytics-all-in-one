...
export default function SchoolDetail({ params }:{ params:{ unitid: string } }){
  const [codes,setCodes]=useState<string[]>(['UG_ENROLL_TOTAL']); const [transform,setTransform]=useState<TransformKind>('level'); const [showTrend,setShowTrend]=useState(true); const [resp,setResp]=useState<ApiResp|null>(null);
  useEffect(()=>{ const p=new URLSearchParams(); if(codes.length) p.set('codes',codes.join(',')); if(transform!=='level') p.set('transform',transform); fetch(`/api/schools/${params.unitid}/metrics?`+p.toString()).then(r=>r.json()).then(setResp); },[params.unitid,codes,transform]);
  ...
  return (<div><h1>{resp?.university?.name??'School'}</h1><p style={{color:'#666'}}>{resp?.university?.city}{resp?.university?.state?`, ${resp?.university?.state}`:''} (unitid {params.unitid})</p>
    ...
    <div style={{marginTop:16,display:'grid',gap:24}}>
      {(resp?.series??[]).map(s=>(<div key={s.metric.code}><TimeSeriesChart series={s.data} title={`${s.metric.name}${transform!=='level'?` — ${transform}`:''}`} unit={s.metric.unit??undefined} showTrend={showTrend} trend={s.trend} /></div>))}
    </div>
  </div>);
}
